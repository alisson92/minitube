...
- **Tornar o repositório público:** descartada nesta decisão — eliminaria a necessidade de credencial, mas exporia publicamente ADRs com detalhes de conta AWS e todo o histórico do projeto. **Atualização, ainda na mesma sessão:** o repositório foi tornado público pelo operador por conveniência operacional (permitir `git pull` direto no CloudShell sem configurar credencial Git lá), depois da deploy key já estar implementada. A deploy key SSH segue funcionando normalmente (funciona em repositório público ou privado) e não foi removida — mas a razão original para descartar "repositório público" (exposição de detalhes de conta AWS) passou a valer na prática. Decisão de manter público ou reverter para privado fica com o operador; não revertida automaticamente nesta sessão.
