...
variable "operator_role_arn" {
  description = "IAM role ARN backing the daily operator's IAM Identity Center permission set, granted cluster-admin via an EKS access entry (module.eks's aws_eks_access_entry.operator). Only changes if the cloudlab-operator permission set is recreated — find it via `aws iam get-role --role-name AWSReservedSSO_cloudlab-operator_<hash> --query Role.Arn --output text` (CloudShell/root, read-only)."
  type        = string
