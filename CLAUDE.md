...
- **Infraestrutura de pé (⚠️ atenção ao fim de sessão):** além do bucket de state e do IAM Identity Center (persistentes por design), a VPC completa do lab (`terraform/envs/lab/`) **está no ar agora**, incluindo o NAT Gateway (recurso com custo por hora). Conforme o princípio de infraestrutura efêmera, isso precisa ser destruído antes de fechar a sessão, salvo decisão explícita em contrário.
- **Próximos passos:** (1) decidir e executar o destroy de `terraform/envs/lab/` ao final desta sessão (ou registrar aqui, explicitamente, por que ficou de pé); (2) abrir/mergear o PR desta branch (`feat/vpc-module`); (3) próximo entregável da Fase 1: EKS com node group spot.
