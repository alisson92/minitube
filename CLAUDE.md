...
- **Persistent-infrastructure decision reverted (2026-08-02) after an unexpected ~R$600 bill.** `terraform/bootstrap-iam/` has already been destroyed and confirmed empty via a follow-up `plan`. `terraform/bootstrap/` (S3 state bucket, two ECR repositories, Route 53 hosted zone, wildcard ACM certificate, the Argo CD deploy key's SSM parameter, and the architecture showcase site — S3 + CloudFront, [ADR 017](docs/adr/017-persistent-architecture-showcase-site.md)) is pending destroy: `prevent_destroy` on the state bucket was removed (see [ADR 001 update](docs/adr/001-terraform-state-backend.md#update--2026-08-02)) to unblock it. Going forward **no MiniTube infrastructure is persistent by design** — everything, including the remote Terraform backend, follows the ephemeral apply/destroy cycle.
