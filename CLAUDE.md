...
- **Próximos passos:** (1) abrir PR de `feat/eks-node-group` para `main`; (2) após o merge, o item que falta para fechar a Fase 1 por completo é o **budget alert** (ainda não implementado — só mencionado como objetivo no `CLAUDE.md`/`000-motivation.md`); (3) seguir para a Fase 2 (API + transcoder) ou para o budget alert, a critério do autor.
- **Regra de custo para esta e futuras fases (CloudFront, etc.):** só o que está em `terraform/bootstrap-iam/` (roles, permission set) e o bucket de state + IAM Identity Center ficam de pé entre sessões, porque não geram custo. Todo o resto — VPC, EKS, CloudFront, e o que vier nas fases seguintes — é criado do zero em `terraform/envs/lab/` no início de cada sessão de teste e destruído por completo ao final dela, sempre com `terraform plan -destroy` revisado antes. Confirmado nesta sessão: ciclo completo apply→validate→destroy do EKS não deixou nada de pé além de `bootstrap-iam`.
