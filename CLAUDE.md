...
- **Pendência registrada — migrar `cloudlab-operator` para AWS IAM Identity Center (SSO):** hoje o operador usa `aws_iam_access_key` (credencial estática de longa duração). A própria AWS recomenda IAM Identity Center para operadores humanos — `aws sso login` gera credenciais temporárias que expiram sozinhas, eliminando a necessidade de rotação manual de access key. Avaliar migração em sessão futura; se adotada, vira ADR (decisão de arquitetura de acesso). Até lá, `aws_iam_access_key` do `cloudlab-operator` permanece sem `prevent_destroy`, justamente para não atritar com rotação manual ocasional.
