...
- **Full teardown completed (2026-08-02) after an unexpected ~R$600 bill.** The persistent-infrastructure decision (ADR 001, 008, 017) is reverted — **no MiniTube infrastructure is persistent by design anymore**; everything, including the remote Terraform backend, now follows the ephemeral apply/destroy cycle. `terraform/bootstrap-iam/`, `terraform/bootstrap/` (S3 state bucket, two ECR repositories, Route 53 hosted zone, wildcard ACM certificate, the Argo CD deploy key's SSM parameter, the architecture showcase site), and `terraform/envs/lab/` are all destroyed and confirmed empty via the AWS CLI (no leftover S3 buckets, ECR repos, hosted zones, ACM certificates, CloudFront distributions, or SSM parameters matching the project). Resuming the project requires re-bootstrapping the remote backend from scratch per [`docs/runbooks/bootstrap/bootstrap-remote-backend.md`](docs/runbooks/bootstrap/bootstrap-remote-backend.md), then re-applying `bootstrap-iam/` and `bootstrap/` before `envs/lab/`. See [ADR 001's update](docs/adr/001-terraform-state-backend.md#update--2026-08-02) for what broke during this teardown and how it was recovered.
