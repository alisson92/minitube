...
- **Próximos passos:** (1) mergear o PR #3 (`refactor/cloudlab-operator-sso-migration`); (2) iniciar o módulo próprio de VPC (subnets públicas/privadas, 1 NAT) em `terraform/envs/lab/`, usando o backend S3 já existente com uma `key` própria (`envs/lab/terraform.tfstate`).
