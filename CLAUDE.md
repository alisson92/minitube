...
Practice, in an integrated way: **Terraform** (the author is preparing for the Terraform Associate certification), **Kubernetes/EKS**, **GitOps with Argo CD**, **observability** (Prometheus, Grafana, Loki, SLOs), and **reliability engineering** (load testing with k6, autoscaling, incident response).
...
- **Repository organization complete** (post-roadmap, in preparation for making the repository public): file/directory names in English (PR #36); own Terraform modules `vpc`/`eks` (PR #37, [ADR 013](docs/adr/013-terraform-vpc-eks-modules.md)); `docs/runbooks/` organized by category (PR #38); `load/README.md` documenting the load-scenario coverage (PR #39); this `CLAUDE.md` rewritten from a session diary into a current-state snapshot (PR #40); an audit of redundant/verbose comments across all `.tf`/`.yaml`/`.sh`/`.js`/`.py` code (PR #41); a documentation audit (3 outdated pieces of content fixed, PR #42); [`docs/runbooks/run-the-project.md`](docs/runbooks/run-the-project.md) — a single runbook covering the full sequence, from account bootstrap to `destroy`, including an explicit caveat about the `bootstrap-iam/` resources that require a root/CloudShell session; and a full repository-wide documentation translation from Portuguese to English (this pass).
- **Root `Makefile` (`make validate-all`):** runs `envs/lab`'s 6 functional checks in dependency order, continuing even if one fails, with a `PASS`/`FAIL` summary per check and a non-zero exit code if any fails — replaces running each `scripts/validate-*.sh` in isolation. Individual targets via `make help` (includes `validate-budget`, kept separate since it's about `bootstrap-iam/`, not `envs/lab`). Result-capture logic tested with fake scripts before wiring up the real ones (2 simulated failures → correct summary and exit code; all passing → exit 0).
- **CI implemented — `.github/workflows/ci.yml`, 4 jobs, static checks only (explicit decision: no AWS credentials in CI, no real `plan`/`apply`):** `terraform fmt`; `terraform validate` + `tflint` matrixed across the 5 directories (`-backend=false`, never touching AWS); `security` (`trivy config` on `terraform/`, `gitleaks` across full history); `lint` (`yamllint`, `shellcheck --severity=warning`, `ruff`, and `scripts/check-markdown-links.py` — the link checker used manually across several reorganization PRs, now automated). Validated end to end by running the real workflow locally via `act`/Docker before the first push — not just the individual commands.
  Real findings fixed while setting this up (no pre-existing comment/documentation was wrong, but the process of turning on lint for the first time exposed this): a dead `GRAFANA_PORT` in `validate-observability.sh` (shellcheck); unsorted imports in the 3 `app/api/` files (ruff); missing `required_version` in the `vpc`/`eks` modules (tflint — added as `>= 1.5.0`, more permissive than root's `~> 1.14`, by design); comment spacing in 2 `values.yaml` files (yamllint); and a real bug in trivy's official install script (`contrib/install.sh` resolves its download via `get.trivy.dev`, which 404'd for the tested version) — worked around by downloading the binary straight from GitHub Releases, the same pattern already used for shellcheck/tflint/gitleaks in the workflow.
  `.trivyignore` and `.gitleaks.toml` each document exactly why every finding is accepted (most were already a deliberate trade-off recorded in an ADR — lab cost/simplicity, not an oversight).
- **Next steps (optional, no formal phase attached):**
  ...
  4. ~~Make the repository public for portfolio/LinkedIn~~ — **done.** The repository is now public; the architecture showcase site's "View the source" link works for visitors.
