...
- **Infraestrutura de pé:** bucket de state `minitube-tfstate-479213212405` (versionado, criptografado AES256, sem acesso público, `prevent_destroy` ativo), criado e verificado na conta AWS `479213212405`. O acesso operacional está em transição: o usuário IAM estático `cloudlab-operator` (policy `PowerUserAccess`) ainda existe na AWS, mas já foi **removido do código** de `terraform/bootstrap-iam/` — falta aplicar o destroy + o novo desenho via CloudShell (ver pendência abaixo).
...
- **Módulos de bootstrap separados:** `terraform/bootstrap/` (bucket S3, uso diário liso com `cloudlab-operator`, local) e `terraform/bootstrap-iam/` (agora modela um Permission Set + account assignment do IAM Identity Center, não mais usuário/access key — só roda com sessão root/CloudShell, pois `PowerUserAccess` e os recursos `aws_ssoadmin_*`/`aws_identitystore_*` continuam fora do alcance do operador diário). Separação de módulos feita via `terraform state mv` na Fase 1, mantida no ADR 003. Ver ADR 002 e ADR 003.
...
- **Profile local:** `AWS_PROFILE=cloudlab` configurado como IAM user estático (`aws sts get-caller-identity` validado), ainda **não migrado** para `sso-session`/`aws sso login` — falta executar os passos 4-9 do runbook `aws-account-bootstrap.md` (habilitar IAM Identity Center, criar usuário, aplicar `bootstrap-iam`, reconfigurar o profile).
- **Feito até aqui:** Fase 0 commitada e enviada; conta AWS nova criada e bootstrapada (bucket de state + operador IAM) via AWS CloudShell; ADR 001, ADR 002 atualizados; **código da migração para IAM Identity Center escrito** (`terraform/bootstrap-iam/` reescrito, ADR 003 criado, runbook `aws-account-bootstrap.md` atualizado) — falta a execução manual (console) e o `terraform apply` via CloudShell.
- **Próximos passos:** (1) executar a migração para SSO — habilitar IAM Identity Center, criar usuário `alisson.cloudlab@gmail.com`, `terraform apply` em `bootstrap-iam/` via CloudShell, reconfigurar `~/.aws/config` com `sso-session`, validar `terraform plan` limpo em `bootstrap/` via `AWS_PROFILE=cloudlab`; (2) só depois iniciar o módulo próprio de VPC (subnets públicas/privadas, 1 NAT) em `terraform/envs/lab/`, usando o backend S3 já existente com uma `key` própria (`envs/lab/terraform.tfstate`).
- **Pendência resolvida — migração para SSO:** decisão tomada e código escrito nesta sessão (ADR 003). Não é mais uma pendência em aberto de arquitetura, apenas execução pendente (ver Próximos passos acima).
