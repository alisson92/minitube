...
- **Fase atual:** 4 — Borda, DNS e TLS (Fase 3 encerrada por completo)
...
- **Nota de segurança registrada nesta sessão:** o repositório GitHub (`alisson92/minitube`) foi tornado **público** pelo operador (conveniência de `git pull` direto no CloudShell) — decisão do operador, não revertida automaticamente. Isso expõe publicamente todo o histórico e os ADRs (incluindo detalhes reais de conta AWS: account ID, ARNs de roles, nomes de recursos) — nenhuma credencial/segredo commitado, mas é uma mudança real de superfície de exposição frente ao que o ADR 007 havia originalmente descartado. Ver atualização registrada no ADR 007.
...
- **Próximos passos:** (1) decidir se o repositório volta a ser privado, agora que a deploy key está funcionando (a conveniência do CloudShell não depende mais de o repo ser público); (2) abrir PR de `feat/argocd-bootstrap` para `main`; (3) após o merge, iniciar a Fase 4 (CloudFront + Route 53 + external-dns + cert-manager com o domínio próprio).
