...
- **Infraestrutura de pé:** só o bucket de state e o IAM Identity Center (`cloudlab-operator` + a role `minitube-network-smoke-test`), ambos persistentes por design. A VPC do lab (`terraform/envs/lab/`) foi **destruída ao final da sessão** (`terraform destroy`, `plan -destroy` revisado antes) — confirmado via `aws ec2 describe-vpcs` (VPC e NAT Gateway não existem mais na conta). Princípio de infraestrutura efêmera respeitado.
...
- **Módulo de EKS escrito, ainda NÃO aplicado na AWS** (branch `feat/eks-node-group`, ADR 004): `terraform/bootstrap-iam/main.tf` ganhou as roles `minitube-eks-cluster-role` e `minitube-eks-node-role` (+ service-linked roles condicionais) e a policy inline única do permission set (`operator_pass_smoke_test_role` renomeada para `operator_pass_roles`, unificando os `Statement` — a API do Identity Center só aceita uma inline policy por permission set). `terraform/envs/lab/eks.tf` (novo) define `aws_eks_cluster` (`authentication_mode = "API"`), `aws_eks_node_group` gerenciado spot (`2x t3.medium`, min=1/max=3/desired=2) e `aws_iam_openid_connect_provider` (IRSA, criado antecipadamente). Tags `kubernetes.io/cluster/minitube-lab=shared` adicionadas às subnets existentes. `terraform fmt`/`validate` passaram em ambos os módulos; **nenhum `apply` foi executado** — falta a checagem manual dos service-linked roles via CloudShell/root, o `apply` de `bootstrap-iam`, o `apply` de `envs/lab` e rodar `scripts/validate-eks.sh` (runbook em `docs/runbooks/validate-eks-cluster.md`).
- **Próximos passos:** (1) sessão root/CloudShell — verificar `AWSServiceRoleForAmazonEKS`/`AWSServiceRoleForAmazonEKSNodegroup` e ajustar `create_eks_service_linked_roles` se já existirem; (2) `apply` de `terraform/bootstrap-iam`; (3) `apply` de `terraform/envs/lab` (via `AWS_PROFILE=cloudlab`) — como a VPC da sessão anterior já foi destruída, este `apply` recria **tudo do zero** (VPC + cluster + node group + OIDC provider), não é incremental; (4) rodar `scripts/validate-eks.sh` e confirmar todos os PASS; (5) `terraform destroy` de `envs/lab` ao final da sessão — remove VPC, cluster, node group e OIDC provider por igual, fechando o critério de conclusão da Fase 1; (6) abrir PR de `feat/eks-node-group` para `main`.
- **Regra de custo para esta e futuras fases (EKS, CloudFront, etc.):** só o que está em `terraform/bootstrap-iam/` (roles, permission set) e o bucket de state + IAM Identity Center ficam de pé entre sessões, porque não geram custo. Todo o resto — VPC, EKS, CloudFront, e o que vier nas fases seguintes — é criado do zero em `terraform/envs/lab/` no início de cada sessão de teste e destruído por completo ao final dela, sempre com `terraform plan -destroy` revisado antes.
