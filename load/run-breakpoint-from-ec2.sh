...
# run-breakpoint.sh, for the find-or-create-video step). k6 itself is
# installed on the remote instance, not required locally.
...
# Stage durations in breakpoint.js are fixed (~17m total) regardless of
# PEAK_RATE -- only the target rate scales, not the schedule. Padded well
# past that for install/setup overhead.
...
echo "Uploading breakpoint.js to the instance..."
script_b64=$(base64 -w0 ./k6/breakpoint.js)
run_remote "writing breakpoint.js" \
  "echo ${script_b64} | base64 -d | sudo tee /tmp/breakpoint.js >/dev/null" \
  60

echo "Running k6 breakpoint scenario from EC2 against ${base_url} (video_id=${video_id}, PEAK_RATE=${PEAK_RATE} req/s)..."
run_remote "k6 run" \
  "BASE_URL='${base_url}' VIDEO_ID='${video_id}' PEAK_RATE='${PEAK_RATE}' /usr/local/bin/k6 run --quiet /tmp/breakpoint.js" \
  "$RUN_TIMEOUT_SECONDS"
